...
/**
 * @fileoverview 获取底部单元格类 - 提供单元格下方区域提取功能
 * @description 该模块用于查找指定字符串下方的所有单元格，
 * 返回匹配位置及其下方单元格列表。
...
function GetBottomCellsClass(config) {
   ...
   * 执行查找任务
   * @function doJob
   * @description 遍历表格查找匹配字符串，收集下方所有单元格
   * @returns {Array.<Object>} 结果数组，每项包含col、row和bottomCells属性
   */
  function doJob() {
    var L = [];
   ...
   * 获取下方单元格列表
   * @function doJobA
   * @description 从指定位置向下收集所有有效单元格
   * @param {number} col - 列索引
   * @param {number} row - 行索引
   * @returns {Array.<Object>} 下方单元格位置数组
   */
  function doJobA(col, row) {
    var t = { col: col, row: row };
    var L = [];
    while (true) {
