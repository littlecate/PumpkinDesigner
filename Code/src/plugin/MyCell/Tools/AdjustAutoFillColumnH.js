'use strict';
function AdjustAutoFillColumnH(config) {
    ...
    function doJob() {
        ...
        while (t1 != null) {
            bodyIndex++;
            t1 = findOne(0, t1.endRow + 1, cellSheet.colWidthList.length - 1, cellSheet.rowHeightList.length - 1);            
        }
    }
    ...
    function mergeReportBodyInfos(L) {        
        var maxRowSpan = getMaxRowSpan(L);
    ...
    function findNextReportBodyInfo(t1) {
        var nextRightCell = autoO.getNextRightCell(t1.col, t1.row);
        if (nextRightCell == null) {
            return null;
        }
        return getBodyInfo(nextRightCell.col, nextRightCell.row);
    }
    ...
    function getBodyInfoA(col, row, s) {     
        ...
        while (nextBottomCell != null) {
            ...
            if (!isGoodNextBottomCell(t1, t2, tt1)) {
                break;
            }
    ...
    function isGoodNextBottomCell(t1, t2, tt1) {
